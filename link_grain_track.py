...
for step_data in steps:
    ...
    trip_start_grains = convert_to_dict(step_data['trip_start_grains'])
    # position = convert_to_dict(step_data['track_based_on_position'])
    
    merged_map = {}
    for d in [confident_track, retrack_result, trip_start_grains]: 
        merged_map.update(d) # Merge confident_track, retrack_result and trip_start_grains into single dictionary
...
for initial_id in merged_step_maps[0].keys():
    ...
    full_grain_map[initial_id] = grain_id_links 


print(full_grain_map)

with open("full_grain_map.pkl", "wb") as f:
    pickle.dump(full_grain_map, f)